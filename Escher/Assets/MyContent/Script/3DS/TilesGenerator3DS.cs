...
public class TilesGenerator3DS : MonoBehaviour
{
    public TileData3DS[] tileDataSet;
    public Vector3Int grid;
    public Vector3Int unitSize;
    public Vector3 gridOrigin = Vector3.zero;
    public Transform map;
    private List<Tile3DS> tiles;
    private Cell3DS[,,] gridPossibilities;
    ...
    {
        gridPossibilities = new Cell3DS[width, length, height];
        ...
        for (int x = 0; x < width; x++)
        {
            for (int z = 0; z < length; z++)
            {
                ...
                {
                    gridPossibilities[x, z, y] = new Cell3DS(tiles);
                }
            }
        }
    }
    ...
    //     {
    //         for(int z = 0; z < length; z++){
    //             List<Tile3DS> currentCellPossibilities = gridPossibilities[x, z, 0].PossibleTiles;
    //             for (int i = currentCellPossibilities.Count - 1; i >= 0; i--)
    //             {
    //                 Tile3DS tile = currentCellPossibilities[i];
        ...
        do
        {
            ...
            for (int x = 0; x < width; x++)
            {
                for (int z = 0; z < length; z++)
                {
                    for(int y = 0; y < height; y++)
                    {
                        List<Tile3DS> currentCellPossibilities = gridPossibilities[x, z, y].PossibleTiles;
    ...
    private bool CheckTileConstraints(Tile3DS tile, int x, int z, int y)
    {
        ...
        if (z < length - 1)
        {
            List<Tile3DS>  rightNeighbors = gridPossibilities[x, z + 1, y].PossibleTiles;
            bool hasCompatibleRightNeighbor = false;
            foreach (Tile3DS neighbor in rightNeighbors)
            {
        ...
        if (z > 0)
        {
            List<Tile3DS>  leftNeighbors = gridPossibilities[x, z - 1, y].PossibleTiles;
            bool hasCompatibleLeftNeighbor = false;
            foreach (Tile3DS neighbor in leftNeighbors)
            {
        ...
        if (x > 0)
        {
            List<Tile3DS> backNeighbors = gridPossibilities[x - 1, z, y].PossibleTiles;
            bool hasCompatibleBackNeighbor = false;
            foreach (Tile3DS neighbor in backNeighbors)
            {
        ...
        if (x < width - 1)
        {
            List<Tile3DS> frontNeighbors = gridPossibilities[x + 1, z, y].PossibleTiles;
            bool hasCompatibleFrontNeighbor = false;
            foreach (Tile3DS neighbor in frontNeighbors)
            {
        ...
        if (y > 0)
        {
            List<Tile3DS> bottomNeighbors = gridPossibilities[x, z, y-1].PossibleTiles;
            bool hasCompatibleBottomNeighbor = false;
            foreach (Tile3DS neighbor in bottomNeighbors)
            {
        ...
        if (y < height - 1)
        {
            List<Tile3DS> topNeighbors = gridPossibilities[x, z, y+1].PossibleTiles;
            bool hasCompatibleTopNeighbor = false;
            foreach (Tile3DS neighbor in topNeighbors)
            {
    ...
    public void Generate()
    {
        while (!IsGenerationComplete())
        {
            ...
            //Debug.Log(lowestEntropyCell.x+" "+lowestEntropyCell.y+" "+lowestEntropyCell.z);
            List<Tile3DS> possibleTiles = gridPossibilities[lowestEntropyCell.x, lowestEntropyCell.y, lowestEntropyCell.z].PossibleTiles;
            //Debug.Log(possibleTiles.Count);
            Tile3DS selectedTile = possibleTiles[Random.Range(0, possibleTiles.Count)];
    ...
    private void PlaceTile(Tile3DS tile, int x, int z, int y)
    {
        outputGrid[x, z, y] = Instantiate(tile.tileData.tileObject, new Vector3(x*unitSize.x, y*unitSize.y, z*unitSize.z), Quaternion.Euler(new Vector3(tile.tileData.rotation.x,tile.tileData.rotation.y,tile.tileData.rotation.z)), map);
    }

    public void Analyze(){
        tiles = new List<Tile3DS>();
        addTiles(tileDataSet.Length);
        for(int i=0; i< tiles.Count; i++){
            // Debug.Log(tiles[i].tileData.tileObject + " rotateX" + tiles[i].tileData.rotation.x + " rotateY" + tiles[i].tileData.rotation.y + " rotateZ" + tiles[i].tileData.rotation.z);
            // Debug.Log("Front    "+tiles[i].tileData.front[0] + "    " +tiles[i].tileData.front[1]);
            // Debug.Log("Back    "+tiles[i].tileData.back[0] + "    " +tiles[i].tileData.back[1]);
            // Debug.Log("Right    "+tiles[i].tileData.right[0] + "    " +tiles[i].tileData.right[1]);
            // Debug.Log("Left    "+tiles[i].tileData.left[0] + "    " +tiles[i].tileData.left[1]);
            // Debug.Log("Top    "+tiles[i].tileData.left[0] + "    " +tiles[i].tileData.left[1]);
            // Debug.Log("Bottom    "+tiles[i].tileData.left[0] + "    " +tiles[i].tileData.left[1]);
    ...
    private void addTiles(int tileNum){
        for(int i=0; i<tileNum; i++){
            tiles.Add(new Tile3DS(tileDataSet[i]));
        }
        if(rotationY){
            for(int i=0; i<tileNum; i++){
                Tile3DS current1 = new Tile3DS(tiles[i].tileData);
                current1.rotateY90();
                tiles.Add(current1);
                Tile3DS current2 = new Tile3DS(current1.tileData);
                current2.rotateY90();
                tiles.Add(current2);
                Tile3DS current3 = new Tile3DS(current2.tileData);
                current3.rotateY90();
                tiles.Add(current3);
            }
        }
        if(rotationXZ){
            for(int i=0; i<tileNum*4; i++){
                ...
                current3.rotateX90();
                tiles.Add(current3);
                Tile3DS current4 = new Tile3DS(tiles[i].tileData);
                current4.rotateZ90();
                tiles.Add(current4);
                Tile3DS current5 = new Tile3DS(current4.tileData);
                current5.rotateZ90();
                current5.rotateZ90();
                tiles.Add(current5);
            }
